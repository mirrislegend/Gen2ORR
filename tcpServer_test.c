...
int GetSocketOptions(int socket) {  
// print the buffers to the screen
	int SocketInfo;  
	...
	SocketInfo = getsockopt(socket, SOL_SOCKET, SO_SNDBUF, &sndbuf, &optlen);  
	if ( SocketInfo ) {  
		displayError("getsockopt(socket,SOL_SOCKET,"   
			     "SO_SNDBUF)");  
	}  
	...
	SocketInfo = getsockopt(socket, SOL_SOCKET, SO_RCVBUF,  &rcvbuf, &optlen);  
	if ( SocketInfo ) {  
		displayError("getsockopt(socket,SOL_SOCKET,SO_RCVBUF)");  
	}  
...
//print the table contents
	printf("table filled: \n");
	   ...
	   {
		printf("%d, ::channelport: %d chanbuff: %s numsub: %d socket: %d \n", 
		alpha, setTable[alpha].channelport, setTable[alpha].chanbuff, 
		setTable[alpha].numsub, setTable[alpha].channelsocket);
	   }
		//setTable[i].channelport=34000+i; //a port number
		//setTable[i].chanbuff[0] = '\0'; //a buffer
		//setTable[i].numsub=0;		//a count of the number of subscribers
...
int main(int argc, char const *argv[]){
	...
	while(1)
	{
		...
		/*   for (int alpha = 0; alpha<10; alpha++ )
		   {
			printf("%d, ::channelport: %d numsub: %d name: %d  \n", 
		        alpha, table[alpha].channelport, table[alpha].numsub, 
			table[alpha].channel_name);
		   }
		*/
